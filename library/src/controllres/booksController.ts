...
import idGenerator from "node-unique-id-generator";
import fs from 'fs';
import { updateCounter, readCounter } from '../helpers/utils';
...
class BooksController {
  ...
  async createBookPost(req: Request, res: Response): Promise<void> {
    try {
      const {
        title,
        description,
  ...
  async updateBookGet(req: Request, res: Response): Promise<void> {
    try {
      const { id } = req.params;
      const findBook = await bookSchema.findById(id);
      ...
      console.error(error);
      res.status(500).send(`Ошибка сервера при редактировании книги`);
    }
  }

  async updateBookPost(req: Request, res: Response): Promise<void> {
    try {
      const {
        title,
        description,
      ...
      res.status(500).send('Ошибка сервера при сохранении исправлений в книге');
      console.log(error);
    }
  }
}
